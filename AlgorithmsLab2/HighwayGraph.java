 ...
 *
 * Starter code for Lab 2 by 
 * @author Jim Teresco
 *
 * Please add the names of your group members here.
 */
...
public class HighwayGraph
{
    ...
    // construct from a TMG format file that comes from the givem
    ...
    public static void main(String args[]) throws IOException {
        ...
        int east = 0;
        int west = 0;
        for (int i = 1; i < g.vertices.length - 1; i++)
        {
        ...
        System.out.println("Longest vertex: " + g.vertices[longest].label);
        System.out.println("Shortest vertex: " + g.vertices[shortest].label);
        System.out.println("Most northern vertex: " + g.vertices[north].label);
        System.out.println("South-most vertex: " + g.vertices[south].label);
        System.out.println("Furthest to the west vertex: " + g.vertices[west].label);
        System.out.println("Furthest to the east vertex: " + g.vertices[east].label + "\n");

        // Edge Search
        Edge longer = g.vertices[0].head;
        Edge shorter = g.vertices[0].head;
        int count = 0;
        ...
        for (Vertex v : g.vertices)
        {
            Edge e = v.head;
            while (e != null) {
                // compares the length of e with shorter and longer; if the length is shorter than shorter or longer than longer
                // the pointers are adjusted accordingly
                if (e.length > longer.length)
                {
                    longer = e;
                }
                if (e.length < shorter.length)
                {
                    shorter = e;
                }
                count++;
        ...
        System.out.println("Longest edge label: " + longer.label);
        System.out.println("Shorest edge label: " + shorter.label);
        System.out.println("Number of edges in graph: " + g.numEdges);
